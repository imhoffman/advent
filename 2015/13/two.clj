...
(def amended-happiness-vecs
  (let [op (set (for [e input-happiness-vecs] (first e)))]
    (loop [stack op
           out   input-happiness-vecs]
      (if (empty? stack)
        out
        (recur
          (rest stack)
          (-> out
             (conj ,, (vector "Me" "gain" "0" (first stack)))
             (conj ,, (vector (first stack) "gain" "0" "Me"))))))))

(def peeps (set (for [e amended-happiness-vecs] (first e))))
...
(def happiness-table  ;;  as in tabulation, not dinner table
  (loop [vecs  amended-happiness-vecs
